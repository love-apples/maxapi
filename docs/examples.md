...
!!! note "Ограничения на загружаемые файлы"
    ...
    проверяются одновременно — должны выполняться оба.

    Библиотека при отправке через `bot.upload_media` или
    `attachments` проверяет только размер файла и лишь
    предупреждает в логе (`bot`) при его превышении, не выбрасывая
    исключение — окончательную проверку выполняет сервер MAX.
