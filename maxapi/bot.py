...
class Bot(BaseConnection):
    ...
    async def upload_media(
        ...
        https://dev.max.ru/docs-api/methods/POST/uploads). Перед
        отправкой библиотека проверяет только размер файла и лишь
        предупреждает в логе `bot` при его превышении, исключение
        не выбрасывает — окончательную проверку (включая формат,
        разрешение изображения и длительность аудио) выполняет
        сервер MAX и может вернуть ошибку загрузки.
