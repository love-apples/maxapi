...
Файл содержит примеры SAMPLE_URLS. Отредактировать для добавления новых.
...
SAMPLE_URLS = {
    # Проверить ключи. Совпадающие заменят данные, новые добавят.
    # --
    ...
    if USE_AUTOMATIC_EXPECTED:
        ...
        if "file" in cfg:
            finfo = await inspector.inspect_file(
                cfg["file"], full_read_threshold=0
            )
        elif "url" in cfg:
